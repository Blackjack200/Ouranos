...
public class DownstreamLoginHandler implements BedrockPacketHandler {
    ...
    public DownstreamLoginHandler(
        KeyPair keyPair,
        ProxyServerSession downstream,
        ...
        if (System.getenv("USE_XBOX") != null) {
            try {
                accessToken = XboxLogin.getAccessToken(
                    System.getenv("XBOX_ACCOUNT"),
                    System.getenv("XBOX_PASSWORD")
                );
            } catch (Exception e) {
    ...
    private ChannelFuture connect() {
        return Ouranos.getOuranos()
            .prepareUpstreamBootstrap()
            .option(
                RakChannelOption.RAK_PROTOCOL_VERSION,
                Ouranos.REMOTE_CODEC.getRaknetProtocolVersion()
            )
            .handler(
                new BedrockChannelInitializer<ProxyClientSession>() {
                    @Override
                    protected ProxyClientSession createSession0(
                        BedrockPeer peer,
                        int subClientId
                    ) {
                        return new ProxyClientSession(
                            (CustomPeer) peer,
                            subClientId
                        );
                    }
    ...
    private ChannelFuture connectProxy() {
        return Ouranos.getOuranos()
            .prepareUpstreamBootstrap()
            .handler(
                new TcpChannelInitializer<ProxyClientSession>() {
                    @Override
                    protected ProxyClientSession createSession0(
                        BedrockPeer peer,
                        int subClientId
    ...
    private void initSession0(ProxyClientSession upstream) {
        ...
        session.movement().inputMode = InputMode.from(
            Integer.parseInt(clientData.get("CurrentInputMode").toString())
        );
        ...
        log.info(
            "{}[{}({})] connected to the remote server through {}, {}=>{}",
            DownstreamLoginHandler.this.identityData.displayName(),
            downstream.getPeer().getSocketAddress(),
            downstreamProtocolId,
            upstream.getPeer().getChannel().getClass().getSimpleName(),
            downstreamProtocolId,
            upstreamProtocolId
        );
        ...
        CompletableFuture.supplyAsync(
            () -> {
                ...
                return new UpstreamInitialHandler(
                    session,
                    assembleLoginPacket(session)
                );
            },
            Ouranos.getOuranos().getScheduler()
        )
            .thenAcceptAsync(handler -> {
                ...
                session
                    ...
                    .getChannel()
                    .eventLoop()
                    .execute(() -> session.setUpstreamHandler(handler));
            })
            .exceptionally(ex -> {
                if (!session.isAlive()) {
                    return null;
                ...
                log.error(
                    "Error while initializing data mapping/assemble login packet",
                    ex
                );
                session.disconnect(
                    "Error while initializing data mapping/assemble login packet: " +
    ...
    private LoginPacket assembleLoginPacket(OuranosProxySession session) {
        var newClientData = LoginPacketUtils.writeClientData(
            this.keyPair,
            session,
            this.identityData,
            this.clientData,
            Ouranos.getOuranos().getConfig().login_extra
        );
        ...
        if (System.getenv("USE_XBOX") != null) {
            ...
            newLogin.setAuthPayload(
                new CertificateChainPayload(chain, AuthType.FULL)
            );
            ...
            session.setIdentity(
                new AuthData(extraData.displayName, extraData.xuid)
            );
        } else {
            newLogin.setAuthPayload(this.authPayload);
            session.setIdentity(this.identityData);
        }
        newLogin.setClientJwt(newClientData);
        newLogin.setProtocolVersion(
            session.getUpstream().getCodec().getProtocolVersion()
        );
        return newLogin;
    }
}
